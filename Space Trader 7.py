...
# Travel from planet to planet buying and selling goods, but watch our for space pirates!
...
def create_planets() -> None:
    for row in range(len(planetgood)):                  # for each planetgood[] row
        ...
        for good in range(len(goods_names)):                     # for each item in goods_names[]
            planetgood[row][good] = random.randint(0, 20)      # amount varies with each turn
            planetprice[row][good] = random.randint(10, 30)    # price set for the game
...
def market() -> None:
    ...
    while bs != 'M':
        ...
        print(f"Cargo = {myship.cargo_hold} units, ({myship.max_cargo_hold} units max).")
...
def leave_planet() -> None:
    ...
    if random.randint(0, 10) > 0: space_pirates()      # random chance of space pirates
    planet_goods_changes()


def planet_goods_changes() -> None:
    for row in range(len(planet_names)):      # for each planet[] location is first item in row
        for n in range(len(goods_names)):                       # for each item in goods_names[]
            planetgood[row][n] = random.randint(0, 20)      # amount varies with each turn. Static prices
...
def ship_maintenance() -> None:
    ...
    print(f"\nMaintenance options:")
    print(f"0. Buy a bigger ship!     $100")
    print(f"1. Upgrade laser power    $ 25")
    print(f"2. Increase cargo hold    $ 15")
    print(f"3. Get better shields     $ 25")
    print(f"4. Repair the shields     $ 15")
    print(f"5. Fill up with fuel      $ 15")
    print(f"6. Bigger fuel tank       $ 20")
    s = ''
    while s != 'M':
        print(f"\nYou have ${myship.money} in the bank.")
        s = input("Item to upgrade, or (M)ain menu: ").upper()
        if s == '0': upgrade_ship(100)
        if s == '1': upgrade_laser(25)
        if s == '2': upgrade_cargo(15)
        if s == '3': upgrade_shields(25)
        if s == '4': repair_shields(15)
        if s == '5': fill_with_fuel(20)
        if s == '6': upgrade_fuel_tank(15)
...
def upgrade_laser(cost) -> None:
    ...
    myship.laser += 3
    myship.money -= cost
    print(f"Upgraded laser from {myship.laser - 5} to {myship.laser}.")
...
def upgrade_ship(cost) -> None:
    ...
    if (myship.shiptype + 2) == len(ship_names):
        ...
        return
    myship.shiptype += 2
    myship.cargo_hold += 10
    myship.max_fuel += 10
    myship.max_shields += 10
    myship.laser += 10
    myship.money -= cost
    print(f"Your new ship is {ship_names[myship.shiptype]} (ship_names[{myship.shiptype + 1}])")  # see shipnames[]
...
print("goods, but watch our for space pirates!")
